...
# define ROM_Bconstat(dev)				Bconstat(dev)
# define ROM_Bconin(dev)				Bconin(dev)
# define ROM_Bconout(dev,c)				Bconout(dev,c)
# define ROM_Setexc(vnum,vptr)				Setexc(vnum,vptr)
# define ROM_Bcostat(dev)				Bcostat(dev)
# define ROM_Kbshift(data)				Kbshift(data)

# define ROM_Initmous(type,param,vptr)			Initmous(type,param,vptr)
# define ROM_Getrez()					Getrez()
# define ROM_Iorec(ioDEV)				Iorec(ioDEV)
# define ROM_Rsconf(baud,flow,uc,rs,ts,sc)		Rsconf(baud,flow,uc,rs,ts,sc)
# define ROM_Keytbl(nrml,shft,caps)			Keytbl(nrml,shft,caps)
# define ROM_Cursconf(rate,attr)			Cursconf(rate,attr)
# define ROM_Settime(time)				Settime(time)
# define ROM_Gettime()					Gettime()
# define ROM_Offgibit(ormask)				Offgibit(ormask)
# define ROM_Ongibit(andmask)				Ongibit(andmask)
# define ROM_Dosound(ptr)				Dosound(ptr)
# define ROM_Vsync()					Vsync()
# define ROM_Bconmap(dev)				Bconmap(dev)

# define ROM_VsetScreen(lscrn,pscrn,rez,mode)		VsetScreen(lscrn,pscrn,rez,mode)
