...
long fp_alloc	(struct proc *p, FILEPTR **resultfp, int len, const char *func);
void fp_done	(struct proc *p, FILEPTR *fp, short fd, char fdflags, const char *func);
void fp_free	(FILEPTR *fp, const char *func);

# define FP_ALLOCN(p, result, len) fp_alloc  (p, result, len, __FUNCTION__)
# define FP_ALLOC(p, result)       fp_alloc  (p, result, 0, __FUNCTION__)
