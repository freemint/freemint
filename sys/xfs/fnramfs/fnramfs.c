...
# define MSG_MINT	\
	"\033pMiNT to old, fnramfs NOT installed!\033q\r\n"

# define MSG_FAILURE(s)	\
	"\7Sorry, fnramfs NOT installed: " s "!\r\n\r\n"
...
__creat (COOKIE *d, const char *name, COOKIE **new, unsigned mode, int attrib)
{
	...
	*new = kmalloc (sizeof (**new));
	...
	{
		DEBUG (("fnramfs: __creat: kmalloc fail!"));
		r = ENOMEM;
	}
...
__unlink_cookie (COOKIE *c)
{
	...
	if (c->stat.nlink > 0)
	{
		DEBUG (("fnramfs: __unlink_cookie: nlink > 0, inode not deleted"));
	}
	else
	{
		if (c->links > 0)
		{
			DEBUG (("fnramfs: __unlink_cookie: inode in use (%li), not deleted", c->links));
		}
		else
		{
			DEBUG (("fnramfs: __unlink_cookie: deleting unlinked inode"));
...
{
	static struct fs_descr fs_descriptor = { &ftab };
	long r;
	...
	if ((MINT_MAJOR == 0)
		|| ((MINT_MAJOR == 1) && ((MINT_MINOR < 15) || (MINT_KVERSION == 0))))
	{
		c_conws (MSG_MINT);
		goto failure;
	}
	...
	/* install ramdisk */
	r = d_cntl (FS_INSTALL, INSTALL, (long) &fs_descriptor);
	if (r != (long) k)
	{
	...
	/* mount ramdisk */
	r = d_cntl (FS_MOUNT, INSTALL, (long) &fs_descriptor);
	if (r >= 0 && r == fs_descriptor.dev_no)
	{
		static char initial [] = "dynamic ram-xfs ½ by [fn]";
		STAT *s;
		...
		s		= &(root->stat);
		s->dev		= fs_descriptor.dev_no;
		s->ino		= (long) root;
		s->mode		= S_IFDIR | DEFAULT_DMODE;
		s->nlink	= 1;
		/* uid		= 0; */
		/* gid		= 0; */
		s->rdev		= fs_descriptor.dev_no;
		s->atime.time	= CURRENT_TIME;
		s->mtime.time	= CURRENT_TIME;
		s->ctime.time	= CURRENT_TIME;
		/* size		= 0; */
		...
		{
			c_conws (MSG_FAILURE ("out of memory"));
			goto failure;
		}
		
		DEBUG (("fnramfs.c: ok (dev_no = %i, root->stat.dev = %i)",
			fs_descriptor.dev_no, root->stat.dev));
...
ram_lookup (fcookie *dir, const char *name, fcookie *fc)
{
	...
	DEBUG (("fnramfs: ram_lookup fail (name = %s, return ENOENT)", name));
	return ENOENT;
}
...
ram_rename (fcookie *olddir, char *oldname, fcookie *newdir, const char *newname)
{
	...
	{
		DEBUG (("fnramfs: ram_rename: cross device rename: [%c] -> [%c]!", olddir->dev+'A', newdir->dev+'A'));
		return EXDEV;
	}
...
ram_fscntl (fcookie *dir, const char *name, int cmd, long arg)
{
	switch (cmd)
	{
		...
		case V_CNTR_WP:
		{
			...
			{
				s->flags |= MS_RDONLY;
				ALERT (("Ext2-FS [%i]: remounted read-only!", dir->dev));
			...
			{
				s->flags &= ~MS_RDONLY;
				ALERT (("Ext2-FS [%i]: remounted read/write!", dir->dev));
...
ram_lseek (FILEPTR *f, long where, int whence)
{
	...
	{
		DEBUG (("fnramfs: ram_lseek: leave failure EBADARG (where = %li)", where));
		return EBADARG;
	}
