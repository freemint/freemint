...
mouse_int (void)
{
	...
	if(change)
	{
		...
		if((actlen >= 6) && (mse_data.new[0] == 1)) /* report-ID */
		{
			...
			mouse_packet[0] = ((mse_data.new[1] & 1) << 1) + ((mse_data.new[1] & 2) >> 1) + 0xF8;
		...
		else /* boot report */
		{
			...
			mouse_packet[0] = ((mse_data.new[0] & 1) << 1) + ((mse_data.new[0] & 2) >> 1) + 0xF8;
...
mouse_probe(struct usb_device *dev)
{
	...
	if ((ep_desc->bmAttributes &
		...
		mse_data.ep_int = ep_desc->bEndpointAddress &
	...
	mse_data.irqinterval = (mse_data.irqinterval > 0) ? mse_data.irqinterval : 255;
	mse_data.irqpipe = usb_rcvintpipe(mse_data.pusb_dev, (long)mse_data.ep_int);
...
#ifndef TOSONLY
	long r = kthread_create(get_curproc(), mouse_poll_thread, NULL, NULL, "mousepoll");
        if (r)
        {
		return 0;
	}
...
#ifdef TOSONLY
//	old_ikbd_int = Setexc (0x70/4, (long) interrupt_ikbd);
	{
		ret = Super(0L);
