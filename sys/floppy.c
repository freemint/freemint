...
{
	return floppy_rw(rwflag, (signed long)buffer, number, recno, floppies[dev].spt, floppies[dev].sides, dev);
}
...
signed long floppy_block_io(DI *di, int rwflag, void *buffer, ulong size, long lrecno)
{
	...
	return floppy_rw(rwflag,(signed long)buffer, count, lrecno, floppies[di->drv].spt, floppies[di->drv].sides, di->drv);
}
...
#if 0 /* freemint syncing with EmuTOS */
void protobt(signed long buf, signed long serial, signed short type, signed short exec)
{
  signed short is_exec;
  ...
  case 0:
    setisigned short(b->bps, SECT_SIZ);
    b->spc = 1;
    setisigned short(b->res, 1);
    b->fat = 2;
    setisigned short(b->dir, 64);
    setisigned short(b->sec, 360);
    b->media = 252;
    setisigned short(b->spf, 2);
    setisigned short(b->spt, 9);
    setisigned short(b->sides, 1);
    setisigned short(b->hid, 0); 
    break;
  case 1:
    setisigned short(b->bps, SECT_SIZ);
    b->spc = 2;
    setisigned short(b->res, 1);
    b->fat = 2;
    setisigned short(b->dir, 112);
    setisigned short(b->sec, 720);
    b->media = 253;
    setisigned short(b->spf, 2);
    setisigned short(b->spt, 9);
    setisigned short(b->sides, 2);
    setisigned short(b->hid, 0); 
    break;
  case 2:
    setisigned short(b->bps, SECT_SIZ);
  ...
  case 0:
    ...
    break;
  case 1:
    setisigned short(b->cksum, 0);
    cksum = compute_cksum(buf);
    setisigned short(b->cksum, 0x1234 - cksum);
    break;
  default:
    /* unknown */
  }
...
#if 0 /* FreeMiNT: we do not need it, but remains for compatibility */
static void setisigned short(uchar *addr, ushort value)
{
  addr[0] = value;
  addr[1] = value >> 8;
}
