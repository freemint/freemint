 ...
 */
int
watch_object(LOCK lock, XA_TREE *wt, int ob, int in_state, int out_state)
{
	OBJECT *dial = wt->tree;
	...
	else
	{
		...
		while (mb)
		{
			/* This loop contains a pretty busy wait, but I don't
			 * think it's to much of a problem as the user is
			 * interacting with it continuously.
			 */
			(void) Syield();
...
unsigned long
XA_graf_dragbox(LOCK lock, XA_CLIENT *client, AESPB *pb)
{
	RECT r, last, dist;
...
unsigned long
XA_graf_rubberbox(LOCK lock, XA_CLIENT *client, AESPB *pb)
{
	RECT r, d = {0};
...
unsigned long
XA_graf_watchbox(LOCK lock, XA_CLIENT *client, AESPB *pb)
{
	XA_TREE *wt;
...
unsigned long
XA_graf_slidebox(LOCK lock, XA_CLIENT *client, AESPB *pb)
{
	short d;
...
unsigned long
XA_graf_growbox(LOCK lock, XA_CLIENT *client, AESPB *pb)
{
	CONTROL(8,1,0)
...
unsigned long
XA_graf_movebox(LOCK lock, XA_CLIENT *client, AESPB *pb)
{
	int w = pb->intin[0];
	int h = pb->intin[1];
	int x = pb->intin[2];
 ...
 */
unsigned long
XA_graf_mouse(LOCK lock, XA_CLIENT *client, AESPB *pb)
{
	short m = pb->intin[0];
...
unsigned long
XA_graf_handle(LOCK lock, XA_CLIENT *client, AESPB *pb)
{
	CONTROL(0,5,0)
...
unsigned long
XA_graf_mkstate(LOCK lock, XA_CLIENT *client, AESPB *pb)
{
	CONTROL(0,5,0)
