...
int
set_scroll(XA_CLIENT *client, OBJECT *form, int item)
{
	OBJECT *ob = form + item;
	SCROLL_INFO *sinfo = XA_calloc(&client->base, 1, sizeof(SCROLL_INFO), 105, 0);
...
void
click_scroll_list(LOCK lock, OBJECT *form, int item, struct moose_data *md)
{
	SCROLL_INFO *list;
	SCROLL_ENTRY *this;
	OBJECT *ob = form + item;
...
void
dclick_scroll_list(LOCK lock, OBJECT *form, int item, struct moose_data *md)
{
	SCROLL_INFO *list;
	SCROLL_ENTRY *this;
	OBJECT *ob = form + item;
