...
static bool menu_title(LOCK lock, XA_WINDOW *wind, XA_WIDGET *widg, int locker);
static XA_TREE *set_popup_widget(Tab *tab, XA_WINDOW *wind, OBJECT *form, int item);

static bool click_menu_widget(LOCK lock, struct xa_window *wind, struct xa_widget *widg);
...
static bool
is_attach(XA_CLIENT *client, OBJECT *menu, int item, XA_MENU_ATTACHMENT **pat)
{
	...
	if (at == NULL || (attach_to->ob_flags&SUBMENU) == 0)
		return false;
 ...
 */
int
inquire_menu(LOCK lock, XA_CLIENT *client, OBJECT *tree, int item, MENU *mn)
{
 ...
 */
int
attach_menu(LOCK lock, XA_CLIENT *client, OBJECT *tree, int item, MENU *mn)
{
	int ret = 0;
	XA_MENU_ATTACHMENT *at = client->attach;
	OBJECT *attach_to;
...
int
detach_menu(LOCK lock, XA_CLIENT *client, OBJECT *tree, int item)
{
	int ret = 0;
...
void
remove_attachments(LOCK lock, XA_CLIENT *client, OBJECT *menu)
{
	if (menu)
	{
		int f = 0;
...
static XA_CLIENT *menu_regcl[MAX_MREG];
...
static OBJECT *
built_desk_popup(LOCK lock, short x, short y)
{
	int n = 0, i = 0,
	    xw = 0, obs, split = 0;
	OBJECT *ob;
	...
	while (client)
	{
		if ((client->client_end && client->type == APP_ACCESSORY)
		    || client == C.Aes)
		{
			menu_regcl[n] = client;
...
menu_finish(struct task_administration_block *tab)
{
	...
	if (tab)
	{
		...
		DIAG((D_menu, NULL, "[%d]menu_finish, ty:%d\n", tab->dbg2, tab->ty));
...
{
	DIAG((D_menu,NULL,"collapse tab:%lx, upto:%lx\n", tab, upto));
	...
	else
	{
		...
		if (m != k->point_at_menu)
		{
			if (   k->point_at_menu > -1
			    && !(k->root[k->point_at_menu].ob_state & DISABLED))
			{
				change_entry(tab, 0);
			}
			k->point_at_menu = m;
...
click_menu_entry(struct task_administration_block *tab)
{
	...
	DIAG((D_menu, NULL, "click_menu_entry: stage=%d\n",k->stage));
	if (k->stage != IN_MENU)
	{
	...
	else
	{
		...
		DIAG((D_menu, NULL, "%sABLED,ti=%d,ab=%d,kc=%d,ks=%d,m=%d\n",d ? "DIS" : "EN",
		...
		{	
			...
			else if (subm || (!subm && (kc != root[titles].ob_head || m == about)))
			{
				...
				if (wt->owner != C.Aes)
				{
					...
					DIAG((D_menu, NULL, "to %s,title=%d,point_at=%d\n", t_owner(wt), kc, m));
...
click_popup_entry(struct task_administration_block *tab)
{
	...
	if (md->mn_item >= 0 && (k->root[md->mn_item].ob_state & DISABLED) != 0)
		md->mn_item = -1;
	
	DIAG((D_menu, NULL, "click_popup_entry %lx + %d\n", md->mn_tree, md->mn_item));
...
click_form_popup_entry(struct task_administration_block *tab)
{
	...
	if (item >= 0 && (k->root[item].ob_state & DISABLED) != 0)
		item = -1;

	DIAG((D_menu, NULL, "click_form_popup_entry %lx + %d\n", k->root, item));
 ...
 */
static bool
display_menu_widget(LOCK lock, struct xa_window *wind, struct xa_widget *widg)
{
...
static bool
click_menu_widget(LOCK lock, struct xa_window *wind, struct xa_widget *widg)
{
	XA_TREE *wt = widg->stuff;
	int pid = C.AESpid;
...
#if 0
	{
		if (cfg.menu_locking)
		{
			unlock_mouse(C.Aes,4);
			unlock_screen(C.Aes,4);
		}
	}
#endif
	return false;
...
static bool
menu_title(LOCK lock, XA_WINDOW *wind, XA_WIDGET *widg, int locker)
{
	RECT r;
	Tab *tab;
	MENU_TASK *k;
	...
	if (   k->clicked_title > -1
	    ...
	    && !(root[k->clicked_title].ob_state & DISABLED))
	{
...
void
set_menu_widget(XA_WINDOW *wind, XA_TREE *menu)
{
 ...
 */
static XA_TREE *
set_popup_widget(Tab *tab, XA_WINDOW *wind, OBJECT *form, int item)
{
	MENU_TASK *k = &tab->task_data.menu;
...
void
fix_menu(OBJECT *root, bool desk)
{
	int titles, menus, tbar, s_ob, t_ob;

	DIAG((D_menu, NULL, "fixing menu 0x%lx\n", root));
