...
struct xa_window * get_topwind(int lock, struct xa_client *client, struct xa_window *startw, bool not, WINDOW_STATUS wsmask, WINDOW_STATUS wsvalue);
struct xa_window * next_wind(int lock);
struct xa_client * previous_client(int lock, short exlude);
...
void set_active_client(int lock, struct xa_client *client);

void set_reiconify_timeout(int lock);
void cancel_reiconify_timeout(void);
//void block_reiconify_timeout(void);
//void unblock_reiconify_timeout(void);

void hide_app(int lock, struct xa_client *client);
void unhide_app(int lock, struct xa_client *client);
void hide_other(int lock, struct xa_client *client);
void unhide_all(int lock, struct xa_client *client);
void set_unhidden(int lock, struct xa_client *client);
void recover(void);

//XA_TREE *find_menu_bar(int lock);
struct xa_client * next_app(int lock, bool with_window_or_menu, bool no_accessories);
struct xa_client *	find_desktop (int lock, struct xa_client *client, short exlude);
...
bool any_hidden(int lock, struct xa_client *client, struct xa_window *exclude);
