...
keyboard_menu_widget(enum  locks lock, struct xa_window *wind, struct xa_widget *widg)
{
	...
	{
		MENU_TASK *k = &TAB_LIST_START->task_data.menu;
		if (k->entry)
			k->entry(TAB_LIST_START, -2);
	}
	return false;
}
