...
_my_ioctl (int fd, int cmd, void *arg)
{
	...
	switch (cmd)
	{
		case HDIO_GETGEO:
		{
			...
			g->heads = 255;
			g->sectors = 63;
			g->cylinders = xhdi_sectors / 255 / 63;
			g->start = 0;
...
{
	union { const void *cv; void *v; } bufptr; bufptr.cv = buf;
	u_int32_t recno;
	int32_t ret;
	...
	ret = rwabs_xhdi (1, bufptr.v, size, recno);
	if (!ret)
	{
		xhdi_pos += size;
		return size;
...
msdos_signature (struct sector *s) {
    if (*(unsigned short *) (s->data + 0x1fe) != BSWAP16 (0xaa55)) {
...
typedef struct { __u8 h,s,c; } chs; /* has some c bits in s */
chs zero_chs = { 0,0,0 };
...
struct partition
{
	__u8	bootable;		/* 0x80 - active */
	__u8	begin_head;		/* starting head */
	__u8	begin_sector;		/* starting sector */
	__u8	begin_cyl;		/* starting cylinder */
	__u8	sys_type;		/* What partition type */
...
static chs
get_begin_chs (struct partition *p)
{
	chs a;
	a.h = p->begin_head;
	a.s = p->begin_sector;
...
static chs
get_end_chs (struct partition *p)
{
	chs a;
	a.h = p->end_head;
	a.s = p->end_sector;
...
static void
put_begin_chs (struct partition *p, chs a)
{
	p->begin_head = a.h;
	p->begin_sector = a.s;
	p->begin_cyl = a.c;
...
static void
put_end_chs (struct partition *p, chs a)
{
	p->end_head = a.h;
	p->end_sector = a.s;
	p->end_cyl = a.c;
...
#define PNO(p) pnumber(p, z)
	...
	for(p = partitions; p < partitions+partno; p++)
	  if (p->size) {
		 ...
		 && (BSWAP32 (p->p.start_sect) >= cylindersize)) {
...
extended_partition(char *dev, int fd, struct part_desc *ep, struct disk_desc *z) {
	...
	for (i=0; i<4; i++,p++) {
		...
		partitions[pno].start = next = start + BSWAP32 (p->start_sect);
		moretodo = 1;
	    } else {
		partitions[pno].start = here + BSWAP32 (p->start_sect);
	    }
	    partitions[pno].size = BSWAP32 (p->nr_sects);
	    partitions[pno++].p = *p;
        }
	here = next;
    }
...
msdos_partition(char *dev, int fd, unsigned long start, struct disk_desc *z) {
    ...
    { unsigned int sig = BSWAP16 (*(unsigned short *)(s->data + 2));
      if (sig <= 0x1ae
	  && *(unsigned short *)(s->data + sig) == BSWAP16 (0x55aa)
	...
	partitions[pno].start = start + BSWAP32 (pt->start_sect);
	partitions[pno].size = BSWAP32 (pt->nr_sects);
	partitions[pno].ep = 0;
	partitions[pno].p = *pt++;
    }
...
write_partitions(char *dev, int fd, struct disk_desc *z) {
	...
	*(unsigned short *)(&(s->data[0x1fe])) = BSWAP16 (0xaa55);
    }
    if (save_sector_file) {
...
read_stdin(unsigned char **fields, unsigned char *line, int fieldssize, int linesize) {
    ...
    union { unsigned char *uc; char *sc; } lp, ip; // *lp, *ip;
    int c, fno;
    ...
    lp.sc = fgets((char *)line + 2, linesize, stdin);
    if (lp.sc == NULL) {
	eof = 1;
	return RD_EOF;
    }
    if (!(lp.sc = index(lp.sc, '\n')))
      fatal("long or incomplete input line - quitting\n");
    *lp.sc = 0;

    /* remove comments, if any */
    if ((lp.sc = index((char *)line+2, '#')) != 0)
      *lp.sc = '\0';

    /* recognize a few commands - to be expanded */
    if (!strcmp((char *)line+2, "unit: sectors")) {
	specified_format = F_SECTOR;
	return RD_CMD;
    }

    /* dump style? - then bad input is fatal */
    if ((ip.sc = index((char *)line+2, ':')) != 0) {
	struct dumpfld *d;
      ...
      nxtfld:
		...
		if(!strncmp(ip.sc, d->fldname, strlen(d->fldname))) {
		    ip.sc += strlen(d->fldname);
		    while(isspace(*ip.sc))
		      ip.sc++;
		    if (d->is_bool)
			fields[d->fldno] = line;
			...
			while(isspace(*++ip.sc)) ;
			fields[d->fldno] = ip.uc;
			while(isalnum(*ip.sc)) 	/* 0x07FF */
			  ip.sc++;
		    } else
    ...
    if (lp.uc == (unsigned char *)fields[fno-1])
      fno--;
    return fno;
}
...
static unsigned long
first_free(int pno, int isextended, struct part_desc *ep, int format,
    ...
    if (ep) {
	if (boxes == NESTED || (boxes == CHAINED && !isextended))
	  pp = ep;
...
static unsigned long
max_length(int pno, int isextended, struct part_desc *ep, int format,
