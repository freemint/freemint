...
WINDOW *create_window(const char *title, int kind, 
								int wx, int wy, int ww, int wh, 	/* Grîûe zum îffnen */
								int max_w, int max_h)				/* max. Grîûe */
{
	WINDOW	*v;
	GRECT 	full;
	int 		centerwin = 0;
...
/*
 * Da die Fenster-Dialoge von der CF-Lib verwaltet werden, mÅssen wir
 * dafÅr sorgen, daû sie auch in unserer Fenster-Liste auftauchen (fÅr ^W)
*/
