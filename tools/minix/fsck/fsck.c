 ...
 * Copyright 1992,1993,1994 S.N. Henson, all rights reserved.
 * Version 0.1 
 */
...
static void	traverse_zones	(int (*func)(zone_nr *zone, ushort level));
static int	pass1		(zone_nr *zone, ushort level);
static int	pass1a		(zone_nr *zone, ushort level);
static void	add_dup		(zone_nr zone);
static void	do_dup		(zone_nr zone);
...
static void	lookup_name	(inode_stat *in);
static int	i_to_name	(dir_struct *dir, ushort entry);
static int	mklost		(void);
static void	check_dots	(void);
static int	pass2		(dir_struct *dir, ushort entry);
static int	_pass2		(dir_struct *dir, ushort entry);
static int	pass2a		(dir_struct *dir, ushort entry);
static void	fix_dots	(void);
static void	next_init	(void);
static void	next_inode	(void);
static void	read_inode_init	(void);
static void	read_inode	(void);
static long	alloc_inode	(void);
static void	fix_root	(void);
...
static inode_stat	*ist;
static inode_stat	*inode_status;
...
do_fsck (void)
{
	...
	i = 2+Super->s_zmap_blks+Super->s_imap_blks
						+(Super->s_ninodes+IPB-1)/IPB;

	if( Super->s_firstdatazn < i ) sfatal("First Data Zone too small");

	/* Maybe using new protection ? */
	if( Super->s_firstdatazn > i ) 
	{
		uchar *tblock;
		long secsize;
		tblock = malloc(BLOCKSIZE);
		if(!tblock) fatal("No memory for boot sector buffer");
		read_zone(0,tblock);
		secsize = tblock[0xb]+(tblock[0xc]<<8);
		if( ((secsize & 1023 ) == 0))
		{
			secsize/=1024;
			i+= 2 * secsize - 1;
	...
	if( Super->s_firstdatazn != i ) fprintf(stderr,"Warning: First data" 
		" zone block %d, expected %ld\n",Super->s_firstdatazn,i);
	...
	zbitmap=(ushort *)( ( (long)ibitmap )+ BLOCKSIZE*Super->s_imap_blks);
	szbitmap=(ushort *)( ( (long)zbitmap ) + BLOCKSIZE*Super->s_zmap_blks);
	...
	for (tll = zinums; tll; tll = tll->next) {
	  if ( (tll->member > ROOT_INODE) && (tll->member <= maxino)) 
				inode_status[tll->member-1].flag |= I_ZAP;
	  else fprintf (stderr, "Invalid inode number %ld\n", tll->member);
	}
	...
	for(cino=ROOT_INODE;cino<=maxino;cino++,ist++)
	{
		...
		switch(rip->i_mode & I_TYPE)
		{
			...
			if(rip->i_nlinks)
			{
				printf("Non zero link count for Free inode %ld - ",cino);
			...
			case I_OLDLINK:
			inerr("Old style symbolic link - ");
			if(ask("Convert?","Converted"))
			{
				rip->i_mode &= ALL_MODES;
				rip->i_mode |= I_SYMLINK;
				cdirty=1;
			}
			/* Fall through */
			...
			default:
			printf("Unknown Mode 0%o Inode %ld - ",rip->i_mode,cino);
	...
	if(berr)
	{
		printf("Zone Bitmap : %ld Errors - ",berr);
	...
	if(berr)
	{
		printf("Inode Bitmap : %ld Errors - ",berr);
		if(ask("Install A New Map?","Fixed"))
		{
...
static void
traverse_zones (int (*func)(zone_nr *zone, ushort level))
{
	zone_nr i,j;
	...
	if(flag_trunc!=2) flag_trunc=0; /* 2 means silently truncate (directory) */
	done_trunc=0;
	dirsparse=0;
...
pass1 (zone_nr *zone, ushort level)
{
	...
	{
		printf("Zone number out of range in inode %ld - ",cino);
	...
	else if(mark_zone(*zone)) add_dup(*zone);
	return 0;
}
...
static int
pass1a (zone_nr *zone, ushort level)
{
...
check_root (void)
{
	...
	if((rip->i_mode & I_TYPE)!=I_DIRECTORY)
	{
		...
		if(!incr && ask("Check anyway?","Continuing...")) oddroot=1;
		else
		{
		  if(!incr)
		  {
...
#ifdef V1
	/* Linux compatability */
	if(incr==2 && Super->s_magic==SUPER_V1)
	{
		fprintf(stderr,
		    "Old Style Superblock for V1, increment 2 filesystem - ");
		if(ask("Convert?","Converted"))
		{
			Super->s_magic=SUPER_V1_30;
			write_zone(1,Super);
		}
	}
#endif
	if(oddroot)
	{
		fprintf(stderr,"Root Inode not directory but otherwise OK - ");
...
static int
addfunc (dir_struct *dir, ushort entry)
{
	if(entry<2) return 0;
...
static int
i_to_name (dir_struct *dir, ushort entry)
{
...
check_dots (void)
{
	...
	if(chk_range(rip->i_zone[0]))
	{
		...
		if(strcmp(dir->d_name,"."))
		{
		    printf("No '.' in directory inode %ld - ",cino);
		    if(ask("Fix?","Fixed"))
		    {
		...
		if(dir->d_inum!=cino)
		{
			printf("Bad '.' Entry in directory inode %ld - ",cino);
			if(ask("Fix?","Fixed"))
			{
				dir->d_inum=cino;
				dirty=1;
...
static int
pass2 (dir_struct *dir, ushort entry)
{
	int ret;
...
static int
_pass2 (dir_struct *dir, ushort entry)
{
	inode_stat *iarr;
...
static int
pass2a (dir_struct *dir, ushort entry)
{
	inode_stat *iarr;
	...
	if(iarr->flag & I_FOUND)
	{
		printf("Bad Hard Link to Directory in Inode %ld - ",cino);
		if(ask("Remove?","Removed"))
		{
			dir->d_inum=0;
			iarr->links++;
