...
Window	*wp;
{
	...
	wind_set(wp->id, WF_NAME, (long)title >> 16, (long)title & 0xffff, 0, 0);
	...
	tree[6].ob_height = 8;
        ...
        form_dial(FMD_START, wp->x, wp->y, wp->w, wp->h, wp->x, wp->y, wp->w, wp->h);
	objc_draw(tree, 0, MAX_DEPTH, wp->x, wp->y, wp->w, wp->h);
        form_dial(FMD_FINISH, wp->x, wp->y, wp->w, wp->h, wp->x, wp->y, wp->w, wp->h);
}
...
Window	*wp;
{
	...
	if (wp->id == NO_WINDOW) {
		...
		wp->id = wind_create(VSLIDE|NAME|CLOSER|MOVER, wp->x, wp->y, wp->w, wp->h);
