...
copy_sysroot() {
	...
	mkdir -p "$SYSROOT/bin"
	cp "$SRC/tools/crypto/crypto" "$SYSROOT/bin"
	cp "$SRC/tools/fdisk/fdisk" "$SYSROOT/bin"
	cp "$SRC/tools/fdisk/sfdisk" "$SYSROOT/bin"
	cp "$SRC/tools/lpflush/lpflush" "$SYSROOT/bin"
	cp "$SRC/tools/minix/fsck/fsck.minix" "$SYSROOT/bin"
	cp "$SRC/tools/minix/minit/minit" "$SYSROOT/bin"
	cp "$SRC/tools/minix/tools/flist" "$SYSROOT/bin/mflist"
	cp "$SRC/tools/minix/tools/mfsconf" "$SYSROOT/bin"
	cp "$SRC/tools/mkfatfs/mkfatfs" "$SYSROOT/bin"
	cp "$SRC/tools/mktbl/mktbl" "$SYSROOT/bin"
	cp "$SRC/tools/net-tools/arp" "$SYSROOT/bin"
	cp "$SRC/tools/net-tools/diald" "$SYSROOT/bin"
	cp "$SRC/tools/net-tools/ifconfig" "$SYSROOT/bin"
	cp "$SRC/tools/net-tools/iflink" "$SYSROOT/bin"
	cp "$SRC/tools/net-tools/ifstats" "$SYSROOT/bin"
	cp "$SRC/tools/net-tools/masqconf" "$SYSROOT/bin"
	cp "$SRC/tools/net-tools/netstat" "$SYSROOT/bin"
	cp "$SRC/tools/net-tools/pppconf" "$SYSROOT/bin"
	cp "$SRC/tools/net-tools/route" "$SYSROOT/bin"
	cp "$SRC/tools/net-tools/slattach" "$SYSROOT/bin"
	cp "$SRC/tools/net-tools/slinkctl/slinkctl" "$SYSROOT/bin"
	cp "$SRC/tools/nfs/mount_nfs" "$SYSROOT/bin"
	cp "$SRC/tools/strace/strace" "$SYSROOT/bin"
	cp "$SRC/tools/swkbdtbl/swkbdtbl" "$SYSROOT/bin"
	cp "$SRC/tools/sysctl/sysctl" "$SYSROOT/bin"
